...
class VGGBase(nn.Module):
    ...
    def load_params(self):
        # get list of names of base layer params/bias
        state_dict = self.state_dict()
        param_names = list(state_dict.keys())
        
        # transfer weights/biases from pretrained VGG16.features module
        vgg = vgg16(True)
        pretrained_state_dict = vgg.features.state_dict()
        pretrained_param_names = list(pretrained_state_dict.keys())
        for i, param in enumerate([p for p in param_names[:-4]]):
            assert pretrained_state_dict[pretrained_param_names[i]].shape == state_dict[param].shape, "param shape mis-alignment"
            state_dict[param] = pretrained_state_dict[pretrained_param_names[i]]

        # transfer weights/biases from pretrained VGG16.classifier module
        pretrained_state_dict = vgg.classifier.state_dict()
        # Convert fc6, fc7 to convolutional layers, and subsample (by decimation) to sizes of conv6 and conv7
        # fc6
        weights = pretrained_state_dict['0.weight'].view(4096, 512, 7, 7)  # (4096, 512, 7, 7)
        biases  = pretrained_state_dict['0.bias']  # (4096)
        state_dict['conv6.weight'] = decimate(weights, m=[4, None, 3, 3])  # (1024, 512, 3, 3)
        state_dict['conv6.bias']   = decimate(biases, m=[4])  # (1024)
        # fc7
        weights = pretrained_state_dict['3.weight'].view(4096, 4096, 1, 1)  # (4096, 4096, 1, 1)
        biases  = pretrained_state_dict['3.bias']  # (4096)
        state_dict['conv7.weight'] = decimate(weights, m=[4, 4, None, None])  # (1024, 1024, 1, 1)
        state_dict['conv7.bias']   = decimate(biases, m=[4])  # (1024)

        # load all updated state diction to transfer all pretrained params
        self.load_state_dict(state_dict)

    
    def forward(self, image):
        # conv1 fwd sequence
        out = self.relu1_1(self.conv1_1(image))
        conv1_ft = self.relu1_2(self.conv1_2(out))
        out = self.pool1(conv1_ft)
        # conv2 fwd sequence
        out = self.relu2_1(self.conv2_1(out))
        conv2_ft = self.relu2_2(self.conv2_2(out))
        out = self.pool2(conv2_ft)
        # conv3 fwd sequence
        out = self.relu3_1(self.conv3_1(out))
        out = self.relu3_2(self.conv3_2(out))
        conv3_ft = self.relu3_3(self.conv3_3(out))
        out = self.pool3(conv3_ft)
        # conv4 fwd sequence
        out = self.relu4_1(self.conv4_1(out))
        out = self.relu4_2(self.conv4_2(out))
        conv4_ft = self.relu4_3(self.conv4_3(out))
        out = self.pool4(conv4_ft)
        # conv5 fwd sequence
        out = self.relu5_1(self.conv5_1(out))
        out = self.relu5_2(self.conv5_2(out))
        conv5_ft = self.relu5_3(self.conv5_3(out))
        out = self.pool5(conv5_ft)
        # conv6
        conv6_ft = self.relu6(self.conv6(out))
        conv7_ft = self.relu7(self.conv7(conv6_ft))
        return [conv1_ft, conv2_ft, conv3_ft, conv4_ft, conv5_ft, conv6_ft, conv7_ft]
...
class AuxLayers(nn.Module):
    ...
    def forward(self, conv7_ft):
        # conv8 fwd sequences
        out = self.relu8_1(self.conv8_1(conv7_ft))
        out = self.relu8_2(self.conv8_2(out))
        conv8_2_ft = out
        # conv9 fwd sequences
        out = self.relu9_1(self.conv9_1(out))
        out = self.relu9_2(self.conv9_2(out))
        conv9_2_ft = out
        # conv10 fwd sequences
        out = self.relu10_1(self.conv10_1(out))
        out = self.relu10_2(self.conv10_2(out))
        conv10_2_ft = out
        # conv11 fwd sequences
        out = self.relu11_1(self.conv11_1(out))
        out = self.relu11_2(self.conv11_2(out))
        conv11_2_ft = out
...
class PredLayers(nn.Module):
    ...
    def forward(self, conv4_3_ft, conv7_ft, conv8_2_ft, conv9_2_ft, conv10_2_ft, conv11_2_ft):
        ...
        cl_conv4_3 = cl_conv4_3.view(bs, -1, self.n_classes)   # (N, 5776, n_classes)
        # Conv7 classifier
        ...
        cl_conv7   = cl_conv7.view(bs, -1, self.n_classes)     # (N, 2166, n_classes)
        # Conv8_2 classifier
        ...
        cl_conv8_2 = cl_conv8_2.view(bs, -1, self.n_classes)   # (N, 600, n_classes)
        # Conv9_2 classifier
        ...
        cl_conv9_2 = cl_conv9_2.view(bs, -1, self.n_classes)   # (N, 150, n_classes)
        # Conv10_2 classifier
        ...
        cl_conv10_2 = cl_conv10_2.view(bs, -1, self.n_classes) # (N, 36, n_classes)
        # Conv11_2 classifier
        ...
        cl_conv11_2 = cl_conv11_2.view(bs, -1, self.n_classes) # (N, 4, n_classes)  
...
class SSD300(nn.Module):
    ...
    def __init__(self, n_classes, device=None):
        ...
        # lower level features (conv4_3_feats) have considerably larger scales, we take the L2 norm and rescale
    ...
    def forward(self, image):
        ...
        base_fts = self.base(image)
        conv4_3_feats,  conv7_feats = base_fts[3], base_fts[-1]  # (N, 512, 38, 38), (N, 1024, 19, 19)
        ...
        [conv8_2_feats, conv9_2_feats, conv10_2_feats, conv11_2_feats] = self.aux(conv7_feats)

        # Run prediction convolutions (predict offsets w.r.t prior-boxes and classes in each resulting localization box)
        # (N, 8732, 4), (N, 8732, n_classes)
        locs, classes_scores = self.pred(conv4_3_feats, conv7_feats, conv8_2_feats, 
